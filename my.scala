...
if (args.length > 0) {
	...
	for (line <- Source.fromFile(args(0)).getLines){
		if(line.trim().startsWith("--")){
			...
			var dir = new File(args(1)+line.trim().substring(2,line.trim().lastIndexOf("-d")-1).trim())
			if(dir.exists()){
			...
			lineDir = line
			lineDirTarget = args(1)+line.trim().substring(2,line.trim().lastIndexOf("-d")-1).trim()
		}else{
			
			var sourceFileStr = lineDir.trim().substring(0,lineDir.trim().lastIndexOf("-d")-1).trim()+line.trim()
			...
			for(temp <- tempList){
				if(temp.getName().startsWith(line.trim().substring(0,line.trim().lastIndexOf(".")-1))){
					copyFss(lineDir.trim().substring(0,lineDir.trim().lastIndexOf("-d")-1).trim()+temp.getName(),lineDirTarget)
				}
			}
